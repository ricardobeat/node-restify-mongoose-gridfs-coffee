...
describe 'Friends', ->
    ...
    before (done) ->
        async.parallel [
            ...
            (next) => util.register 'mary', (err, @mary) => next(err)
            (next) => util.register 'tina', (err, @tina) => next(err)
            (next) => util.register 'fafa', (err, @fafa) => next(err)
        ], done
    ...
    it 'should connect friends', (done) ->
        ...
        async.parallel [
            (next) => # John is friends with Mary
                client.post "/friend?#{tokens.john}", { friend: @mary._id }, (err, req, res, obj) ->
                    assert.ifError err
                    assert obj
                    next()

            (next) => # John is friends with Tina
                client.post "/friend?#{tokens.john}", { friend: @tina._id }, (err, req, res, obj) ->
                    assert.ifError err
                    assert obj
                    next()

            (next) => # Mary is friends with Tina
                client.post "/friend?#{tokens.mary}", { friend: @tina._id }, (err, req, res, obj) ->
                    assert.ifError err
                    assert obj
                    next()

            (next) => # Mary is friends with Fafa
                client.post "/friend?#{tokens.mary}", { friend: @fafa._id }, (err, req, res, obj) ->
                    assert.ifError err
                    assert obj
                    next()
        ], done
    ...
    it 'should know john and mary are happy friends', (done) ->
        ...
        async.parallel
            john: (next) -> client.get "/friends?#{q1}", (err, req, res, obj) -> next err, obj
            mary: (next) -> client.get "/friends?#{q2}", (err, req, res, obj) -> next err, obj
        , (err, friendsOf) =>
            assert.ifError err
    ...
    it 'should get mary\'s friends', (done) ->
        ...
        client.get "/friends?#{q}", (err, req, res, friends) =>
            assert.ifError err
    ...
    it 'should get mary and john\'s mutual friends', (done) ->
        q = qs.stringify { token: @mary.token }
        # Mutual friends between Mary and John
        client.get "/friends/common/#{@john._id}?#{q}", (err, req, res, common) =>
            assert.ifError err
